import { generateGroundedText, generateText } from "../llm.js";
...
export async function getJobIssues({
  ...
  const { text: findings, sources = [] } = await research({ system: RESEARCH_SYSTEM, prompt: researchPrompt, apiKey });
  ...
  const raw = await structure({ system: STRUCTURE_SYSTEM, prompt: findings, apiKey });
